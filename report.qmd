...
combined_count <- combined_data %>% group_by(id) %>% summarise(Count = n())
top_3 <- c("ASN00031209","ASN00032040","ASN00040856")

ggplot(combined_count,aes(x = reorder(id,Count),y = Count)) + geom_col() + coord_flip() + theme_classic() + ggtitle("Number of Coastal Taipan sightings \n for each weather station")  + geom_col(data = ~filter(.x, id %in% top_3),fill = "darkred") + geom_label(data = ~filter(.x, id %in% top_3),aes(label = Count),vjust=0.8) + labs(x = "Weather station ID",y = "Number of Coastal Taipan sightings",caption = "Source: Atlas of Living Australia (ALA)")
```
...
narrogin <- aus_stations |>
  filter(id %in% unique(combined_count$id)) |>
  rowwise() |>
...
```{r}
subset_combined_data <- combined_data %>% filter(id %in% top_3)
taipan_weather <- full_join(combined_data,narrogin,by = c("id","eventDate"="date"))
