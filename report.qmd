...
1. Columns datatypes are changed for ease of further analysis such as the case with variable 'eventDate'. \
...
2. **The [galah](https://galah.ala.org.au) package** - available on CRAN and also on GitHub. This package has been instrumental in fetching the data directly into the R environment. 
...
As we are primarily interested in analysing the wild sightings of the Coastal Taipans across Australia, hence, we will filter out the data obtained from museums and animal sanctuaries. This information has been provided in the `event ID` column which will be used to filter the required data as shown in the code chunk below. It has been additionally assumed that the occurences with no mention of the term "museum" have been considered as 

```{r}
taipan <- taipan %>% mutate(occurence = if_else(str_detect(taipan$eventID,"museum"),"Museum","Wild"))
taipan$occurence <- replace_na(taipan$occurence,"Wild")
taipan <- taipan %>% filter(occurence =="Wild") # Selecting only wild occurrences of Coastal Taipans
```

We have observed a total of `r taipan %>% nrow()` occurrences across Australia. However, this number has been obtained from the raw data which __will need to be cleaned further__.
...
taipan <- taipan %>% 
  ...
  filter(!is.na(Latitude)) %>% select(-c("samplingEffort","samplingProtocol"))
save(taipan, file=here::here("data/taipan.rda"))

```

Upon performing the required data cleaning operation, we have a total of `r taipan %>% nrow()` wild sightings of Coastal Taipans across Australia.
