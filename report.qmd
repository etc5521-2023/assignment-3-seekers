...
Note 1: Why we didn't consider Cookstown
Note 2: Each trip is one person as described in the survey method. https://www.tra.gov.au/en/domestic/national-visitor-survey-methodology.html#:~:text=The%20survey%20runs,a%20private%20dwelling.

```{r}
#subset the tourism data to align with the sighting
region_summary <- tourism %>% 
  filter(grepl("Townsville", Region)) %>% 
  group_by(Region) %>% 
  summarize(sum = sum(Trips))
region_summary
```

- The **Townsville City - North Ward** region is the prime candidate for in-depth analysis due to its remarkable lead in the sum of trips, a crucial metric. This data-centric choice prioritizes regions of particular interest for further analysis and significance within the broader analysis. The notably higher number of trips in this region suggests its pivotal role as a major tourism hub as compared to other parts of our study. Also, the month ward is closer to the coastal regions where maximum sightings were recorded as per the part 1 of this report. Focusing on this region is strategic for uncovering valuable insights and patterns of sightings for Australia's Deadly Serpent(The Enigmatic Coastal Taipan). 


```{r}
#subset the data as per Townsville City - North Ward
tourism_subset <- tourism %>% 
  filter(Region == "Townsville City - North Ward")
```

Note 3: The tourism data breaks up only into quarters and not into months, days or hours which is one of the limitations of our temporal analysis. This prevents us from finding daily and seasonal pattterns in the data. 
```{r}
#Temporal Analysis

temporal1 <- tourism_subset %>%
  aggregate(Trips ~ year(Quarter) + Purpose, sum)
graph1 <- temporal1 %>%
  ggplot(aes(x = `year(Quarter)`, y = Trips)) +
  geom_line(alpha = 0.5) +
  geom_smooth()+
...
- A common observation across all purposes is the prominent trend for the year 2020, which is heavily influenced by the global Covid-19 pandemic. As the pandemic swept across the world, a noticeable and significant decline in the number of trips becomes evident. This decline can be attributed to the fact that people were primarily confined to their homes during this period.
- From the variability perspective, the highest degree of variability is observed in the Business category, followed by Holiday and Visiting. However, it's worth noting that the Other category exhibits the least variability among the purposes.
- Shifting our focus to trends, we can observe that the curves for Other and Total appear quite similar showing an upward trend with a decline between 2015 to 2020 which is influenced by the Covid-19 pandemic. However, when examining Holiday, we notice that its curve is notably flat, with limited discernible trends.

```{r}
temporal2 <- tourism_subset %>%
  aggregate(Trips ~ quarter(Quarter) + Purpose, sum)

graph2 <- temporal2 %>%
  ggplot(aes(x = `quarter(Quarter)`, y = Trips)) +
  geom_line(alpha = 0.5) +
  facet_wrap(~ Purpose, scales = "free_y") 
graph2
```

- Based on the above graphs, it can be inferred that the number of trips in quarter one is consistently lower across all travel purposes to Townsville - North Ward.
- Since the trends for the Holiday and Total categories are similar, it is reasonable to assume that a significant proportion of the trips in the Total category have been influenced by the Holiday category.
- This similarity also highlights that the maximum number of trips occurs in the third quarter for each category.
- In all the categories, they initially exhibit an upward trend, followed by a downward slope. However, the Visiting graph stands out with a distinctive trend in the quarterly analysis.

# task 2


```{r}
#mutate the columns year and quarter (Q) in the taipan_wild dataset
taipan_wild_new <- taipan_wild %>% 
  mutate(Year = year(eventDate),
         Q = quarter(eventDate)) %>%
  group_by(Q,Year) %>% 
  summarise(Sightings = n())

#mutate the columns year and quarter (Q) in the tourism_subset dataset
tourism_subset_new <- tourism_subset %>% mutate(Year = year(Quarter),
                                                      Q = quarter(Quarter))

#joining the animal data with the tourism data
joined <- right_join(taipan_wild_new, tourism_subset_new, by = c("Year", "Q"))
joined$Sightings <- ifelse(is.na(joined$Sightings),0,joined$Sightings)
```
...
```{r}
 joined %>%
  ...
  ggplot() +
  geom_line(aes(x = Year, y = trips_total, color = "Trips"), size = 1) +
  geom_line(aes(x = Year, y = total_sightings * 100, color = "Sightings * 100"), size = 1) +
  theme_minimal() +
  scale_color_manual(
    values = c("Trips" = "green", "Sightings * 100" = "red"),
    labels = c("Trips", "Sightings * 100")
  ) +
...
```{r}
 joined %>%
  ...
  summarise(total_sightings = sum(Sightings),
            trips_total = sum(Trips)) %>%
  ggplot() +
  geom_line(aes(x = Q, y = trips_total, color = "Trips"), size = 1) +
  geom_line(aes(x = Q, y = total_sightings * 100, color = "Sightings * 100"), size = 1) +
  theme_minimal() +
  scale_color_manual(
    values = c("Trips" = "green", "Sightings * 100" = "red"),
    labels = c("Trips", "Sightings * 100")
